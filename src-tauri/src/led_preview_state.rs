...
pub struct LedPreviewStateManager {
    state: Arc<RwLock<LedPreviewState>>,
}
...
impl LedPreviewStateManager {
    pub async fn global() -> &'static Self {
        ...
        LED_PREVIEW_STATE_MANAGER
            .get_or_init(|| async {
                let state = match LedPreviewState::read_config().await {
                    Ok(state) => state,
                    Err(e) => {
                        warn!(
                            "Failed to read LED preview state config: {}, using default",
                            e
                        );
                        LedPreviewState::default()
                    }
                };
    ...
    /// Get current LED preview state
    pub async fn get_state(&self) -> LedPreviewState {
        self.state.read().await.clone()
    }

    /// Check if LED preview is enabled
    pub async fn is_enabled(&self) -> bool {
        self.state.read().await.enabled
    }

    /// Set LED preview enabled state
    pub async fn set_enabled(&self, enabled: bool) -> anyhow::Result<()> {
        ...
        info!(
            "LED preview state changed to: {}",
            if enabled { "enabled" } else { "disabled" }
        );

        // 通过WebSocket广播状态变化
        let current_state = self.get_state().await;
        crate::websocket_events::publish_led_preview_state_changed(&current_state).await;

        Ok(())
    }

    /// Toggle LED preview state
